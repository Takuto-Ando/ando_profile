...
現在は、奈良先端大 コンピューティング・アーキテクチャ研究室で開発された、非ノイマン型アーキテクチャであるIMAX (In-Memory Accelerator eXtension)に関わる研究をしています。
IMAXは、リニア粗粒度再構成可能アーキテクチャ (Linear CGRA, CGLA) を基盤としており、これは、高速コンパイルと汎用性によってプログラマビリティを向上させながら、従来の2次元CGRAのスループットとエネルギー効率を維持することができます。

私は、IMAXのプロトタイプでLLMなどのAI演算を実装しています。
